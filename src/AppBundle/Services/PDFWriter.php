...
class PDFWriter
{
    ...
    public function writeTickets($path, $cf, $tickets, $agenda = [], $purchase = null) {
        if(isset($tickets[0])) {
            // We know all tickets are for same event
            $this->write(self::TICKETS_TEMPLATE, $path, ['tickets' => $tickets, 'agenda' => $agenda, 'cf' => $cf, 'purchase' => $purchase]);
        }
    }
