...
class User extends BaseUser implements RecipientInterface, PhysicalPersonInterface
{
    ...
    public function getParticipationToOrganization(Organization $org){
        foreach ($this->participations as $participation){
            if ($participation->getOrganization()->getId() == $org->getId()){
    ...
    public function isInOrganization(Organization $organization) {
        return $this->getParticipationToOrganization($organization) != null;
    }
    ...
    */
    ...
    {
        if (!$this->category_statistics->contains($categoryStatistic)) {
            $this->category_statistics[] = $categoryStatistic;
            $categoryStatistic->setUser($this);
        }
        return $this;
    }
    ...
    public function removeCategoryStatistic(\AppBundle\Entity\User_Category $categoryStatistic)
    {
        if ($this->category_statistics->contains($categoryStatistic)) {
            ...
            $categoryStatistic->setUser(null);
        }
    }
